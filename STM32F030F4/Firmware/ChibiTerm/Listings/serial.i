...
FIFO_Data_t RxBuf[4+sizeof(FIFO)] = { 4-1 };
FIFO_Data_t TxBuf[16+sizeof(FIFO)] = { 16-1 };

void USART_Init(void)
{
	((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x00001000))->APB2ENR |= ((uint32_t)0x00004000);									
  ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->BRR = (25000000UL*2UL)/115200;										



  ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->CR1 = ((uint32_t)0x00000020)|												
								((uint32_t)0x00000004)|((uint32_t)0x00000008)|((uint32_t)0x00000001); 
...
{
	if(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->ISR & ((uint32_t)0x00000020))											
	{
		FIFO_Write((FIFO*)RxBuf,((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->RDR);	
	}
	
	if(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->ISR & ((uint32_t)0x00000080))												
	{
		uint8_t TxD;
		
		if(FIFO_Read((FIFO*)TxBuf,&TxD))
			((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->TDR = TxD;																
		else
			((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800))->CR1 &= ~((uint32_t)0x00000080);									
	}
}
